...
def processAudio():
    ...
    from keras.layers import Dense,LSTM,TimeDistributed, Conv2D, Dropout, Flatten, MaxPooling2D,Conv1D,MaxPooling1D,GlobalAveragePooling1D,AveragePooling1D
    from keras.models import model_from_json
    f0min,f0max=70,600
    unit="Hertz"
    wave_file='Audio5463488.wav'
  ...
  #  mfcc = call(sound , 'To MelSpectrogram...', 0, 0, 0.0001, 0.02, 1.3, 1.6)
    ...
    #new_model = keras.models.load_model('path_to_my_model.h5')
    predict=new_model.predict_classes(df)
    p=''
    if predict==1 :
        p='You may have Parkinson go next level diagnosis'
    else:
        p='Congras Report is Negative'
    data={'df':p}
    data = jsonify(data)
